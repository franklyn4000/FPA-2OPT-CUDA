...
#pragma omp parallel for
    for (int path_i = 0; path_i < paths.population; path_i++) {
        float prevCPath = 0.0f;

        float unsmoothedVertices = 0;

        paths.smoothedPaths[path_i].clear();

        paths.smoothedPaths[path_i].push_back( paths.rawPaths[path_i][0]);
        paths.smoothedPaths[path_i].push_back( paths.rawPaths[path_i][1]);
        paths.smoothedPaths[path_i].push_back( paths.rawPaths[path_i][2]);

        int n =  paths.rawPaths[path_i].size() / 3;
        ...
        for (int i = 1; i < n - 1; i++) {
            P1[0] =  paths.rawPaths[path_i][3 * (i - 1)];
            P1[1] =  paths.rawPaths[path_i][3 * (i - 1) + 1];
            P1[2] =  paths.rawPaths[path_i][3 * (i - 1) + 2];

            P[0] =  paths.rawPaths[path_i][3 * i];
            P[1] =  paths.rawPaths[path_i][3 * i + 1];
            P[2] =  paths.rawPaths[path_i][3 * i + 2];

            P2[0] =  paths.rawPaths[path_i][3 * (i + 1)];
            P2[1] =  paths.rawPaths[path_i][3 * (i + 1) + 1];
            P2[2] =  paths.rawPaths[path_i][3 * (i + 1) + 2];
            ...
            if (distance_PC > std::min(mag_1, mag_2) ||
                c_path + prevCPath > mag_1) {
                //cannot smooth trajectory
                paths.smoothedPaths[path_i].push_back(P[0]);
                paths.smoothedPaths[path_i].push_back(P[1]);
                paths.smoothedPaths[path_i].push_back(P[2]);
                unsmoothedVertices++;
                prevCPath = c_path;
                continue;
            }
            ...
            float cscAlpha = 1 / sin(alpha);
            float oneOverNWaypoints = 1.0f / (n_waypoints - 1);
            ...
            for (int j = 0; j < n_waypoints; j++) {
                ...
                paths.smoothedPaths[path_i].push_back(C[0] - turnRadius * cscAlpha * tau_1[0] * cosAlphaOmega -
                                       turnRadius * cscAlpha * tau_2[0] * cosOmega);
                paths.smoothedPaths[path_i].push_back(C[1] - turnRadius * cscAlpha * tau_1[1] * cosAlphaOmega -
                                       turnRadius * cscAlpha * tau_2[1] * cosOmega);
                paths.smoothedPaths[path_i].push_back(C[2] - turnRadius * cscAlpha * tau_1[2] * cosAlphaOmega -
                                       turnRadius * cscAlpha * tau_2[2] * cosOmega);
        ...
        paths.smoothedPaths[path_i].push_back( paths.rawPaths[path_i][3 * n - 3]);
        paths.smoothedPaths[path_i].push_back( paths.rawPaths[path_i][3 * n - 2]);
        paths.smoothedPaths[path_i].push_back( paths.rawPaths[path_i][3 * n - 1]);

        paths.N_wps[path_i] = unsmoothedVertices / n;
